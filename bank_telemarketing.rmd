...
Our goal on this project is to study a dataset generated in a marketing campaign. This campaign was performed by a portuguese bank between 2008 and 2010 to offer a term diposit. 

We will try to obtain insights to optimize the resources of the marketing department. We will construct as well a predictive model to predict which contacts may result on the subscription to the product that we are offering.
...
The intend in this chapter is not doing an exhaustive review of all the variables on the dataset. It will be centered on the variables with the higher (or lower) predictive power and the variables that may need some preprocessing.
...
4. If we look at the pattern table, we see that when 'housing' is unknown, 'loan' is unknown as well and viceversa
...
We will define a subset of the training set to select the hyperparameters. We'll use it for the algorithms that need more computation power. When possible, we'll use the hole dataset to select hyperparameters.
...
In the next sections we'll train several algorithms in order to predict which contacts will result in a subscription. For each algorithm we may do the next steps:

1. Preprocesing of data: For some algorithms we'll do some operations on the data, as centering and scaling.
2. hiperparameter tunning
3. train the model
...
### GLM with lasso or elasticnet regularization (glmnet)

On the next model we fit two hyperparamenters: alpha and lambda. As this algorithm is quick enough, we'll search the hiperparameters with the whole train set.
...
For this algorithm, the hyperparameter to search is the number of neighbors k.
...
### Naive Bayes
 ...
 Our next algorithm is Naive Bayes. Our search grid includes three hyperparameters: usekernel (true or false), fl (Laplace correction) and adjust (a factor correction on the smoothing bandwidth)
...
The last algorithm that we will train on this project is the Support Vector Machines radial. Here we'll tune two hyperparameters: gamma and C (cost of constraints violation)

```{r}
######################################
## svmRadial
control_svmRadial <- trainControl(
  method="cv", number = 3,
  summaryFunction = twoClassSummary,
  classProbs = TRUE,
  verboseIter = TRUE)

grid_svmRadial <- expand.grid(sigma = c(0, 0.1, 0.2, 0.5, 1), C = c(0.01, 0.1, 0.5, 1, 3))
```

Here we'll apply two preprocessing transformations on the data. Center and scale
...
```{r echo=T, results='hide'}
train_svmRadial <- train(y ~., 
                         data = preProc_train_set[index,], 
                         method = "svmRadial",
                         trControl=control_svmRadial,
                         tuneGrid = grid_svmRadial,
                         tuneLength = 1,
                         metric="ROC")


plot(train_svmRadial)
```
...
fit_svmRadial <- svm(y ~ ., 
                     data=preProc_train_set, 
                     kernel="radial", 
                     cost=train_svmRadial$bestTune$C,
                     sigma=train_svmRadial$bestTune$sigma,
                     probability=TRUE)
```

Finally get the predictions on the test set and calculte AUC: 
...
Finally we'll calculate the mean of the probabilities obtained from the previous models. The intend of this last step is to combine the output of the previous models to obtain better predictions: 
...
## Tradeof between accuracy and sensitibity

To be honest, the results of the ensemble model don't seem quite impressive. Do them? 

But we can still do something. Remember that our objective is to optimize the resources of our marketing department. For example, imagine that the department has a budget to do 1000 calls. We can simply get the probabilities obtained by our ensemble model and select the 1000 calls with the greatest probability to subscribe.

Other approach can be sacrifizing some accuracy in order to gain more sensitibity. We can do this by trying different thresholds with our vector of probabilities. On the next figure we show the effect of selecting different cuts on several performance indicators. For each cut (x axis) we consider y=yes if the probability to be "yes" is greater than the cut.
...
performance_indicators <- function(cut, probs, y) {
  ...
  d = sensitivity-selection_rate
  out = t(as.matrix(c(cut, sensitivity, specificity, accuracy,d, selection_rate)))
  colnames(out) = c("cut","sensitivity", "specificity", "accuracy", "distance", "selection_rate")
  return(out)
...
df_cut %>%
  ...
  geom_line(aes(x=cut,y=distance, color="Distance")) +
  geom_line(aes(x=cut,y=selection_rate, color="Calls selected rate")) +
  labs(color = 'Performance indicators')
```

On this figure, the "Distance" line represents the difference between the ratio of calls where y=yes and the sensitibity (how many subscriptors have we detected)

The best cut is:

```{r}
# best cut
df_cut[which.max(df_cut$distance),]
```

Which is pretty near to the ratio of subscriptions that we have in our dataset. 
